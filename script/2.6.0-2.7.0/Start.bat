...
for %%l in (Chinese,English,Japanese,Korean) do (
    if %%l==Chinese set "N=!path1!"
    if %%l==English set "N=!path2!"
    if %%l==Japanese set "N=!path3!"
    if %%l==Korean set "N=!path4!"
    if exist "!N!" (
        ...
        for /F "usebackq delims=" %%j in ("AudioPatch_%%l_!oldVer!-!newVer!.txt") do (
            for %%k in ("%%j" "%%j.hdiff") do if NOT exist %%k (
                echo "%%k is missing."
                set FileMissing=True
            )
        )
...
if "%PatchFinished%"=="True" (
  ...
  rd /s /q AntiCheatExpert "StarRail_Data\StreamingAssets\Audio\AudioPackage\Windows\DecodedBanks" "StarRail_Data\SDKCaches" "StarRail_Data\webCaches" 2>nul
  del *.bat *.zip *.7z hpatchz.exe hdiffz.exe 7z.exe *.dmp *.bak *.txt *.log
)